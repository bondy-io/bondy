...
def diagram_ingress():
    ...
    hello = (["Busy or memory-high node → immediate",
              "retryable ABORT with wamp.error.unavailable.",
              "Established sessions and post-HELLO",
              "handshakes are unaffected."],
             ["load_regulation.hello.enabled  on",
              "busy signal <- node load monitor",
              "memory-high signal <- node memory monitor",
    ...
    bands = [
        ...
        [("limiter", "http — per source IP, per request",
          ["Every API Gateway, Admin API and MCP",
           ...
           "The OAuth2 endpoints also refuse on a busy",
           "or memory-high node: 503 + retry-after",
          ...
          ["node 100/s·500 (higher than connection)",
           ...
           "-> bondy_rate_limited_total{class, scope}",
           "load_regulation.oauth2.enabled  on"], False),
