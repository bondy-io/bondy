...
* `bondy_connect` returned `wamp.error.internal_error` when a callee handler crashed. The WAMP specification does not define that URI and reserves the `wamp.` namespace; it is now `bondy.error.internal_error`.
