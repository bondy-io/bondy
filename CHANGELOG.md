...
* Sharding and physical layout are configurable per table: realm-partitioned or key-hashed placement, configurable shard counts (`db.main.shard_count`, `db.main.partition_strategy`, `db.main.realm_prefix_depth`; the ephemeral registry database has its own independent shard count, `db.registry.shard_count`), and aggregate-root routing that co-locates related keys (e.g. a user's group memberships) on a single shard for the authentication hot path. Realm identifiers are folded into storage keys so identically-named keys in different realms can never collide.
* The write-ahead log's fsync behaviour is configurable (`db.wal.fsync_mode`: `per_write`, the default, or `batched`, with `db.wal.batched_fsync_interval` / `db.wal.batched_fsync_bytes` tuning the batching window) and its rotation threshold is adjustable (`db.wal.max_segment_bytes`).
...
* Anti-entropy (enabled by default, `db.aae`): pull-based MST reconciliation with bounded per-round page batches, a node-wide concurrency cap (`db.aae.max_concurrency`), a node-wide in-flight page budget (`db.aae.max_pages_in_flight`), adaptive live-sync throttling for quiescent shards (`db.aae.live_sync`), optional load-reactive yielding under routing pressure (`db.aae.load_adaptive`), and exponential retry backoff for failed bootstraps. New and recovering nodes bootstrap by streaming a peer's projection snapshot and then replaying the log delta. A node whose peer has already reclaimed historical pages automatically falls back to a fresh snapshot bootstrap.
* Authentication freshness fence: when replicated security state (credentials, grants) on this node is provably stale beyond `db.aae.fence.max_lag`, authentication is refused rather than decided on stale data. The behaviour on cluster isolation is configurable (`db.aae.fence.on_isolation`); genuine single-node deployments are exempt.
...
* The dealer advertises `progressive_call_results` as a build capability; there is no `bondy.conf` key to turn it on or off. Nothing activates until a client asks for it: a `HELLO` that omits the feature is given `false` rather than the router's advertised value, and a remote callee is gated on its own node's capability rather than on the caller's.
* The `bondy_connect_sdk` Erlang client supports both sides of the feature: `call_async/5` with `receive_progress => true` delivers each progressive result to the caller as `{bondy_connect_client, Token, {progress, Result}}` before the single terminal reply (the synchronous `call/5` rejects the option), and a callee handler invoked for a progressive call receives a `progress` fun in its details whose calls become progressive `YIELD`s. The client's per-call timer follows the same spec semantics (restarted per progressive result, capped by `_deadline`). Call results now also expose the WAMP `RESULT.Details` under the `details` key.

#### Progressive Calls (WAMP Advanced Profile)
* The mirror image of Progressive Call Results: the dealer now implements progressive calls end to end, letting a caller stream a call's **arguments** to the callee in successive chunks under one request id. The caller opens the stream with `CALL.Options.progress = true` and reuses its `CALL.Request` id for every chunk; the dealer forwards each as an `INVOCATION` reusing one invocation id and `Details.progress = true`, and the final chunk (a `CALL` without `progress`) forwards as an `INVOCATION` without it, so the callee learns the input is complete. Chunks reach the callee in send order, including when caller and callee are connected to different cluster nodes (chunks are relayed over the pair's ordered pipeline). Documented in `doc/guides/router/progressive_calls.md`.
* The feature is negotiated end to end but, unlike progressive results, is **not** silently downgraded: because the caller has already begun streaming, a first chunk whose caller or callee did not announce `progressive_calls` in `HELLO` (paired with `call_canceling`, as the specification requires) fails with `wamp.error.option_not_allowed`. Reusing a live request id on a call that is not an open progressive-input stream remains a `wamp.error.protocol_violation`.
* Timeout semantics match progressive results: `CALL.Options.timeout` is the inactivity window between chunks (each chunk restarts it) and the `CALL.Options._deadline` extension (milliseconds) caps the whole call. Cancellation works mid-stream and across the cluster, and a caller whose session ends mid-stream causes the dealer to INTERRUPT (mode `killnowait`) the callee still waiting on its input — local callees directly, remote ones by relaying the cancellation to their node.
* As with progressive results, `progressive_calls` is a build capability with no `bondy.conf` key, and a `HELLO` that omits it is given `false`.
* The `bondy_connect_sdk` Erlang client supports both sides: a caller opens a stream with `call_stream/5` and streams further argument chunks with `send_input/4` / `finish_input/4`, while a callee handler invoked for a progressive call receives an `input` fun in its details that it pulls for each chunk until the fun returns `{last, _, _}`.
...
#### HTTP/2
* HTTP/2 is now served on every listener whose `protocol` is `http`: negotiated via ALPN on a `tls` listener and via the h2c upgrade or prior-knowledge preface on a plaintext one. Resource use is bounded (at most `max_concurrent_streams` concurrent requests per connection, default 100, plus HPACK and frame-rate caps). Note for capacity planning: one HTTP/2 connection can carry up to `max_concurrent_streams` in-flight requests, so `max_connections`-based alarms undercount request-level load.

#### Operator-defined listeners
* A listener is now something an operator declares, by name, rather than one of a fixed set the router hardcoded. `listeners.$name.{transport,protocol,port,path,services,enabled,start_phase}` describes any number of sockets, with TLS as an add-on (`listeners.$name.tls.*`) rather than a separate listener kind, and per-listener option blocks for socket tuning, Cowboy protocol options, CORS, security headers, proxy protocol and the WebSocket/SSE/long-poll carriers. Two names are reserved: `admin`, the administrable endpoint, which the node always provides whether or not it is declared, and an internal Unix-domain socket. Documented in `doc/guides/configuration/listeners.md`.
* A static listener misconfiguration is refused at boot, naming the listener and the reason, rather than producing a node that serves less than the operator asked for: an unknown transport or protocol, an unsupported combination of the two, a missing bind target, a duplicate name or bind, TLS material on a plaintext listener, or a `tls` listener with no certificate. The inventory is resolved as a unit, so any one of these stops the whole node rather than just that listener — which is why a listener with `enabled = off` is exempt from the TLS checks: it will not start, and failing the boot over a socket that never binds would make `enabled = off` mean something other than off. Enabling it runs the same checks at that boot.
* `scripts/migrate_conf.escript` reports and rewrites configuration this release no longer reads, including the removed listener keys, and separately reports listeners a file would lose or declare without an identity, keys it still reads but reads differently, and values it reads but cannot parse. That last kind is not confined to the key it appears on: config generation is all-or-nothing, so a single unusable value discards the whole file and the node boots on whatever the previous successful generation produced, with nothing in the log to say so. Documented in `doc/guides/configuration/checking_your_configuration.md`.
* WAMP broker and dealer **features are no longer `bondy.conf` keys**. A feature is not a toggle: it states which parts of the WAMP advanced profile this build implements. A client asks for a subset in `HELLO`, Bondy intersects that request with its own map, and every message thereafter consults the resulting session flags — an operator was never in that chain, so the thirty `wamp.broker.*` / `wamp.dealer.*` feature keys are gone and the values come from `?BROKER_FEATURES` / `?DEALER_FEATURES` in code. Six advertised values change as a result, each now matching the build rather than a schema default: `payload_passthru_mode` becomes true on both roles, `progressive_calls` and `progressive_call_results` become true (a peer must still announce them explicitly to obtain them), and `broker.reflection` and `dealer.call_reroute` become false — both were announced without an implementation behind them. `scripts/migrate_conf.escript` reports every removed key with the reason.
* Sixteen `flag` settings declared their default as the Erlang term (`{default, true}`) rather than the word the setting accepts (`{default, on}`). The values Bondy runs on are unchanged — a default is parsed by the same code as a value written in `bondy.conf`, and `on` and `true` both resolve to `true` — but `config/bondy.conf.defaults`, which publishes them verbatim, listed values that could not be written back into a `bondy.conf`. It is now a file you can copy from.
* Two new WAMP procedures take a node out of rotation and put it back without a restart or a configuration change: `bondy.listener.suspend` and `bondy.listener.resume`, each taking a phase (`early`, `normal` or `all`). Suspending stops a listener accepting new connections and leaves established ones running, so it drains rather than cuts. Both are callable only from the master realm — whether a node accepts connections is not a per-realm decision — and any value that is not one of the three phases is refused rather than silently matching no listeners. Note that suspending `all` includes the phase carrying `/ping`, `/ready` and `/metrics`, which an orchestrator may read as a dead node; the shutdown path suspends `normal` only, and that is unchanged. Documented in `doc/guides/configuration/listeners.md`.

#### Standardised error reporting
* Every failure Bondy reports to a peer is now one error value with a single, documented payload, projected onto whichever transport is in use. A WAMP `ERROR` carries the error's URI as its `ErrorUri`, one human-readable sentence as `Args[0]`, and the payload as `KWArgs`; an HTTP API Gateway response carries the same payload as its JSON body with a status code derived from the same URI. The payload adds `uri` (the error's identity, and the field to branch on), `nature` (`transient` or `permanent`, so a client can tell a refusal worth retrying from one that is not), `details` (structured context), `causes` (a chain, when one failure caused another), `handle` (a stable support identifier) and `doc_uri`. The pre-existing `code`, `message` and `description` keys keep their values, so existing clients are unaffected. Documented in `doc/guides/reference/error_catalogue.md`.
* An unrecognised internal failure is now reported as `bondy.error.internal_error` carrying only a `trace_id`; the exception reason and stacktrace go to the log under that same `trace_id`. Previously some paths formatted the raw crash reason into the `description` sent to the client.
* Error payloads are JSON-encodable by construction. Terms with no JSON representation (tuples, pids, references, funs, non-UTF-8 binaries) are rendered as text rather than reaching the encoder, and nesting depth and payload size are bounded. Previously such a term raised inside the JSON encoder — after the HTTP status and headers had been chosen, or on the process writing to the WAMP socket.
* Request identifiers and error correlation identifiers are now both W3C Trace Context `trace-id` values (32 lowercase hex characters), generated from a UUIDv7 so they are also time-ordered.
...
* Cowboy upgraded from 2.13.0 to 2.18.0 (with cowlib 2.19.0), picking up upstream security fixes: an HTTP/1.1 parser denial-of-service, HPACK-bomb protection, early rejection of NUL/CR/LF in unexpected positions, and termination of responses carrying invalid header names or values (a response-splitting defence; a response with e.g. control characters in a header value now yields a 500 instead of being relayed). 2.18 adds a bound on the number of cookies parsed from a request (see `max_cookies` below) and a 17-digit ceiling on integer parsing in headers, rejects request URIs carrying a fragment rather than stripping it, rejects a request whose `:authority` and `host` disagree, and fixes a case where a WebSocket frame could exceed `max_frame_size`. Two operator-visible behaviour changes come with it: pipelined HTTP/1.1 requests on one connection are now processed one at a time (pipelining itself still works), and a malformed PROXY protocol header closes the connection quietly instead of producing a crash report.

### Improvements
* Bondy now builds and ships on **Erlang/OTP 29** (`erts-17.0`); the published container images are built from `erlang:29.0.5`. OTP 28 still compiles the tree, so `minimum_otp_vsn` remains `R28`, but the release artefacts and everything tested against them are OTP 29. Three inherited runtime changes are worth knowing before upgrading:
    * TLS listeners now prefer the post-quantum hybrid key-exchange group `x25519mlkem768` and the ML-DSA/SLH-DSA signature algorithms where a peer offers them, and TLS 1.2 always uses RFC 5746 secure renegotiation (the `secure_renegotiate` fallback is gone). Classical groups remain in the default list, so clients that do not offer the hybrid group are unaffected.
    * The `re` module moved from PCRE1 to PCRE2. Its documented incompatibility that can reach a caller is that `bsr_anycrlf`, `bsr_unicode` and `{newline, _}` are now fixed when a pattern is compiled and can no longer be overridden at match time on a pre-compiled regex. Bondy passes none of these, and every pattern Bondy compiles — URI validation, role names, passwords, cookie and path-template parsing — matches identically on both releases.
    * OTP's own `array` serialisation changed representation: a term containing an `array` that was written by an older release with `term_to_binary/1` is not readable by OTP 29. Bondy stores no such term — its on-disk formats are explicit binary encodings — but an external tool that persisted Erlang terms alongside Bondy's data may be affected.
* New Cowboy listener options exposed in `bondy.conf` under `listeners.$name.http.*`: `max_authorization_header_value_length` (raise for large bearer tokens without raising the general header limit), `max_cookie_header_value_length`, `max_authority_length`, `invalid_response_headers` (`error_terminate` | `ignore`) and `max_concurrent_streams`.
* New listener option `listeners.$name.http.max_cookies`: how many cookies a request's `Cookie` header may carry, 100 if unset. A request over the limit is answered with a 400. This bounds the *count*, where the neighbouring `max_cookie_header_value_length` bounds the header's *size* — a single header of legal length can otherwise hold thousands of tiny cookies. Cookie parsing was unbounded before this release.
...
* `bondy_http_connector` (the renamed RPC Gateway) is now observable: telemetry events and Prometheus metrics for requests, retries, token cache hits/fetches/refreshes and secret resolution, plus a new dedicated Grafana dashboard. Every service pool gets a periodic liveness probe (`http_connector.services.$service.liveness.*`, on by default) that raises an alarm after consecutive failures and clears it on recovery — a degrading upstream is now visible before a live WAMP call fails against it.
...
### Fixes
* **An in-VM (`local` transport) client's registrations and subscriptions kept routing after it disconnected.** Every socket transport tears down through `bondy_wamp_protocol:terminate/1`, which performs two cleanups: `bondy_session_manager:close/1` retires the session record, and `bondy_context:close/1` removes the session's registry entries via `bondy_router:flush/2`. The in-VM handler called only the first, so the entries survived; `bondy_session_manager:close/1` also demonitors the owner with `[flush]`, which removes the `DOWN` whose handler would otherwise have flushed them, making the leak permanent rather than merely delayed. A CALL to a departed in-VM callee's procedure was dispatched to a dead process and the caller waited out its timeout with no error to explain it, instead of receiving `wamp.error.no_such_procedure`.
* **Over HTTP long-poll, a router `ABORT` or `GOODBYE` never reached the client; the next request answered `404 transport_not_found` instead.** The transport session enqueues a terminal reply and then stops, but the queue is drained on a `queue_ready` message to itself — which a `gen_server` returning `{stop, ...}` never handles — and `terminate/2` deletes the queue. A failed credential, a rejected realm and a closed session were therefore indistinguishable to a long-poll client, and all three presented as a missing transport with `nature: transient`. The terminal reply is now handed to a poll already parked on the session before it stops, which is what SSE has always done with its stream process. A client whose poller happens to be between cycles at that instant still learns of the close from the next request's 404.
* **A WAMP session over HTTP long-poll or SSE could receive messages out of order.** A transport's output has two producers — the router, and the protocol answering an inbound message synchronously (an inline ERROR, a throttle rejection) — and the two were held in different places: the router's deliveries in `bondy_http_transport_queue`, the synchronous replies in a `reply_buffer` field of `bondy_http_transport_session` which was drained **first**. A reply produced later was therefore delivered before a message queued earlier, which a transport is not allowed to do. Both now go into the one queue, so the client sees the order in which they were produced. Two consequences worth knowing: the synchronous replies now count against `wamp.http_transport.queue.max_messages` and `.max_bytes`, where previously that half was unbounded and a client that posted without ever polling grew it without limit; and they now age out under `.message_ttl` (5 minutes by default) like anything else, where previously they were held for as long as the session lived.
* **An outbound bridge relay that configured no `ping` block crashed its client on start, and one that configured no `reconnect` block never reconnected.** Both come from the same cause: a configuration key's default is used exactly as written, without the key's own nested specification being applied to it, and both of these were written as an empty map — so none of the defaults documented for either block took effect. For `ping`, `bondy_bridge_relay_client:maybe_enable_ping/2` accepts only a block that states `enabled`, so an empty one raised `function_clause` in `init/1`. For `reconnect` there is no crash and nothing in the log: the client simply treated the bridge as having reconnection disabled, though the default has always been enabled with 100 retries. Every `bridge.$name.ping.*` and `bridge.$name.reconnect.*` setting is commented out in the shipped configuration, so a bridge that said nothing about either took exactly those paths. Both blocks are now filled from the specifications that document them. A bridge that set any one key of a block was never affected, because a block that is present is completed key by key.
* The error formatter itself could crash: rendering a `missing_required_value` or `inconsistency_error` whose key was a binary raised `function_clause`, as did any error reason the formatter had no clause for (1-tuples, 4-or-more-tuples, pids, integers, bare lists) — reachable from the dealer's catch-all handler, i.e. from inside an error handler.
* A malformed WAMP `ABORT` reason raised `function_clause` in the connection handler instead of closing the connection with an `ABORT`.
* `WWW-Authenticate` header construction raised `badarg` when the error's `code` was an atom, which it was for several common errors.
* A raw `hackney` reason term and an unrendered upstream error were placed directly into JSON response bodies by the HTTP API Gateway.
* The HTTP long-poll transport answered errors with an empty body; it now returns the standard error payload.
* WAMP error URIs were mapped to HTTP status codes by two separate tables that disagreed with each other; there is now one table.
* `bondy_regulator` passed `erlang:error/3`'s options as a bare tuple rather than a list, so its `error_info` was silently ineffective, and one of its messages named the wrong field.
* `format_error/2` in `bondy_mst` and `bondy_registry` crashed when `error_info` carried no `cause`.
* `bondy_humanized:join/1` emitted a leading `", "` for exactly two items, and crashed for a single non-iodata item.
* `?WAMP_FEATURE_NOT_SUPPORTED` was defined as `wamp.error.no_available_callee` (a copy-paste of the preceding line); it is now `wamp.error.feature_not_supported`.
* `rebar3 dialyzer` failed outright because `rebar.config` still passed the `race_conditions` warning option, removed in OTP 26.
...
* A partially-configured keepalive ping killed the connection instead of being ignored. `ping.enabled` has no default, so a listener or carrier that set some other `ping.*` key and not that one produced a ping block with no `enabled` at all, which every connection handler failed to match — after the socket was accepted, so the listener bound and then died on the first client. An absent `ping.enabled` now means what it reads like: ping off. Stating `ping.enabled = on` and omitting a value the handler needs is still refused at boot, where it is visible, rather than at the first connection.
* A TLS listener that stated any one security header lost HSTS. The configuration renderer completed a partial `security_headers` block with all five members, `hsts` among them as "not set" — and an explicitly-set value wins over the default a TLS listener gets, so naming `frame_options` silently switched HSTS off on exactly the listeners it is for. The renderer no longer completes the block; each consumer supplies its own defaults for the members a listener did not state, which it already did.
* An API Gateway specification's `host` field was silently ignored: a specification declaring `"host": "api.example.com"` answered on every host, and two specifications declaring one path for different hosts collided with the second becoming unreachable. The specification parser groups its rules by host and the pre-existing code compiled them that way; the intermediate step that assembles a listener's table had been flattening them. Hosts are honoured again. Every specification Bondy ships or documents uses `"host": "_"`, so no shipped configuration was affected.
* Bondy's own HTTP paths are now served on every virtual host a specification names. Cowboy's router commits to the first host entry whose host matches and never falls through, so `/ws`, the SSE and long-poll endpoints, `/ping`, `/ready`, `/cluster/topology` and `/metrics` were unreachable on any host a specification declared — including before this release, where they were mounted on the wildcard host as a separate entry. Where a specification declares one of those paths for its own host, the specification answers there and a warning is logged.
* A stored API Gateway specification declaring a path that one of Bondy's own route sets also declares aborted the dispatch-table rebuild with `route_collision` — over a document another node had accepted and replicated. Bondy's route set now wins the path and the shadowed specification route is logged. A collision between two of Bondy's own route sets is still an error: nothing an operator does can produce it.
* An HTTP listener naming a service whose carrier no module implements was accepted at boot and then raised `{no_module_for_carrier, Carrier}` while assembling its dispatch table — inside a listener start or an API-specification rebuild, where no listener name is available to put in the error. It is now refused during listener resolution, naming the listener, the carrier and the service that asked for it.
* **`listeners.$name.linger.timeout` is now in seconds, and a value with a unit suffix means something different than it did.** The key is handed to the OS socket option `{linger, {true, N}}`, whose second component is seconds, but its datatype declared milliseconds — so `linger.timeout = 1s` rendered `1000` and asked the kernel for a **1000-second** blocking close, and that was the shipped default on every raw-socket and bridge-relay listener. A connection whose peer stopped acknowledging held its process and file descriptor for up to about 17 minutes on close. **Action required if you set this key with a unit suffix:** `1s` now means one second, as it reads. A value written as a bare integer already meant seconds and is unaffected, as is the `-1` sentinel. A sub-second value rounds up to one second rather than down to zero, because `{linger, {true, 0}}` means abort the connection and discard unsent data. The `1s` default is restored on top of the corrected unit. Note that `listeners.$name.http.linger.timeout` is a different setting — Cowboy's, not the socket's — and remains in milliseconds. `scripts/migrate_conf.escript check` names this key under `CHANGED MEANING` on any file that sets it, so you do not have to find it by hand.
* Keepalive was off on every raw-socket and bridge-relay listener. The `wamp.{tcp,tls}.ping.*` and `bridge.listener.{tcp,tls}.ping.*` settings defaulted to `enabled = on`, a `10s` probe timeout and 2 attempts, and their per-listener replacements carry no defaults, so no node probed a raw-socket or bridge-relay connection at all — a peer that vanished without a FIN was never detected and a NAT or firewall binding was never held open. All four are supplied again for both protocols. `wamp.tls.ping.max_attempts` defaulted to 3 where `wamp.tcp` and both bridge listeners defaulted to 2; every listener now defaults to **3**, as the transport does not change how many unanswered probes mean a dead peer. That is also the default for a WebSocket connection — `wamp.websocket.ping.max_attempts` was 2 and is now 3 — so one number covers every protocol. **On everything that previously defaulted to 2 (raw-socket TCP, both bridge-relay listeners and WebSocket), a peer that stops answering is therefore dropped one probe later.** The connection closes at `ping.idle_timeout + max_attempts × ping.timeout`, so 50s at the defaults rather than 40s; raw-socket TLS already used 3 and is unchanged. Set `listeners.$name.ping.max_attempts` — or `listeners.$name.websocket.ping.max_attempts` — if you want the shorter deadline back. The outbound half of a bridge relay (`bridge.$name.ping.max_attempts`) moves from 2 to 3 with them, so both ends of a bridge now judge a dead peer the same way.
* An idle raw-socket or bridge-relay connection was never reaped. `wamp.{tcp,tls}.idle_timeout` and `bridge.listener.{tcp,tls}.idle_timeout` defaulted to `8h`, and with the default gone each handler fell back to its own `infinity`. Both are `8h` again.
* The raw-socket keepalive probed on the same timer that reaps an idle connection: the probe interval was read from the listener's `idle_timeout` rather than from `ping.idle_timeout`, so both came due at the same moment and the connection was closed instead of probed — the keepalive could neither hold a NAT binding open nor notice a dead peer any sooner than the reap already did, and at `idle_timeout = infinity` it killed every connection on the listener outright. The probe interval now comes from the ping block, defaulting to `20s`, and is independent of the reap deadline.
* Every HTTP listener except `admin` lost two Cowboy settings: `api_gateway.http*.active_n` defaulted to 100 and fell back to Cowboy's 1 (a throughput regression on the main API port), and `api_gateway.http*.idle_timeout` defaulted to `15s` and fell back to Cowboy's `60s`. The shipped `bondy.conf` templates restate both for `listeners.admin` only, so both regressed on every release. Both defaults are supplied again for any HTTP listener.
* No TLS listener sent an HSTS header. `admin_api.https.security_headers.hsts` supplied `max-age=31536000; includeSubDomains`, and with that mapping gone `bondy_http_security_headers` fell back to `hsts => undefined`. Every TLS HTTP listener now sends it by default; a plaintext listener does not, and an explicit `security_headers.hsts` still wins.
* Reclaiming a projection cell made the next write to that key fail permanently. Projection-cell reclamation deletes a causally-stable dead cell on purpose, but did not tell the causal-context guard that watches for *accidental* loss of the same state — so the write that re-created the cell read an empty context, was indistinguishable from state corruption, and was refused with a `context_regression` error and an error-level log. Reachable at default settings on any add-wins or delete-wins collection whose entries are removed and later re-added, group memberships among them; the guard's protection against genuine loss is unchanged.

### Changes
* **`wamp.transport_queue.*` is renamed to `wamp.http_transport.*`, and `bondy_transport_queue` to `bondy_http_transport_queue`.** Neither name said what the thing is: it is the message queue for the two WAMP transports carried over HTTP — long-poll and SSE — and a WebSocket or raw-socket session never touches it. It exists because those two transports have no process holding a socket for the session's lifetime: a long-poll client is not connected at all between `/receive` calls, so a message routed to it in that gap has nowhere to go unless something parks it. The settings, their units and their defaults are unchanged, and they remain node-wide — one ETS partition ring and one eviction sweeper serve every HTTP transport on the node. **Action required only if you set one of these keys**: `wamp.transport_queue.<key>` becomes `wamp.http_transport.queue.<key>`, and `scripts/migrate_conf.escript` rewrites them for you. One key moves further: `wamp.transport_queue.transport_ttl` becomes **`wamp.http_transport.idle_timeout`**, because it was never a queue setting — it is the session's inactivity deadline, read once by `bondy_http_transport_session`, and it sat in that block only by association. `wamp.transport_queue.overflow_strategy` is removed: it was seeded and read by nothing, its enum admitted a single value, and evicting the oldest entry is the only policy that makes sense for a stream a client reads in order.
* **The global `wamp.websocket.*`, `wamp.sse.*` and `wamp.longpoll.*` blocks were removed.** These were never an alternative spelling of `listeners.$name.{websocket,sse,longpoll}.*` — they were the tier a listener fell back to when it set nothing, which meant one key silently changed the behaviour of every listener at once. A connection setting belongs to the listener the connection arrived on, so that is now the only place to write one. Every key keeps its name and its default under its listener, and the defaults are unchanged: a listener that sets nothing behaves exactly as before. **Action required only if you set one of these keys**: restate it under `listeners.<name>.<carrier>.*` for each listener that serves that carrier, or delete it if it only restated the default. `wamp.websocket.buffer.min`/`.max` are removed outright rather than moved — since Cowboy 2.13 a WebSocket connection inherits its listener's dynamic buffer, so neither key could take effect; `listeners.$name.http.buffer.min`/`.max` are the ones that do. The carrier defaults now live in `bondy_listener_config` and so no longer appear in a generated `etc/bondy.conf`; they are tabulated in the listener configuration reference.
* **`wamp.dealer.pattern_based_registration` and `wamp.broker.pattern_based_subscription` were removed. Both features are always on.** Setting either to `off` did not disable a feature — it stopped the node working. The router registers a wildcard procedure of its own to serve `wamp.session.get`, so with pattern-based registration off that registration was refused and the refusal crashed the session-open path (`{case_clause, {error, pattern_based_registration_disabled}}`), taking down every session on the node. The router also went on advertising both features in `WELCOME` regardless of the setting, so a client had no way to learn they were unavailable. **Action required only if you set either key**: delete the line. `on` was the default and is now the only behaviour; `scripts/migrate_conf.escript` reports both keys and comments them out. `WELCOME` continues to advertise both, so no client sees a change.
* A single security header can now be switched off. `listeners.$name.security_headers.{hsts,frame_options,content_type_options,content_security_policy} = off` suppresses that one header while the others are still sent; `security_headers.enabled = off` remains the all-or-nothing switch. Previously the only way to drop one header was to drop them all, which mattered once HSTS became a default for TLS listeners. Any value other than the literal `off` is still a header value, so `off` cannot collide with `DENY`, `nosniff` or a `max-age=…`.
* Three listener log records changed shape, which matters if you alert on them. The connection-threshold alarms log one `description` of `"Connection threshold exceeded"` with a `threshold_percent` field of `75` or `90`, rather than two separate descriptions naming the percentage; the levels are unchanged (`warning` and `alert`). The drain notice now says it is suspending the normal-phase listeners and that the readiness and metrics endpoints stay up, which is what it always did. A WebSocket connection closed on its idle timeout logs the `idle_timeout` actually in force; every listener now has one, so the earlier `cowboy_default` placeholder is gone.
* Two listeners may now share a port when they bind different addresses. The startup check compared ports alone, so `listeners.a.ip = 10.0.0.1` and `listeners.b.ip = 10.0.0.2` on one port were refused although a socket is identified by its address and port together and the operating system accepts both — the one-certificate-per-interface case the feature exists for. A listener that configures no `ip` binds the wildcard, which covers every address on its port, so it still excludes every other listener there, and that is still refused at boot rather than at bind time. Addresses of different families are treated as overlapping whenever either is a wildcard, which is conservative: `::` and `127.0.0.1` on one port are refused, though whether they collide depends on the host's `bindv6only` setting. Unix-domain socket paths are unaffected — a path clash has no address and stays refused outright.
* An HTTP listener whose `services` list is empty is now refused at boot with the same error as one that declares no `services` key at all. Such a listener bound its socket and answered 404 to every request, with nothing reported anywhere. Note that `listeners.<name>.services =` with no value renders as an empty list, so it is this error rather than a listener with defaults.
* The `api_gateway` and `admin_api` services are now separate carriers rather than two services sharing one. Where a stored API Gateway specification declares a path the built-in Admin API specification also declares on the same listener, the first route assembled still answers it and the other is still unreachable there — unchanged behaviour — but a warning naming the path is now logged instead of nothing. Registering a carrier from an application outside `bondy_router` now takes two environment entries rather than one: `bondy_router.http_services` maps a service to its carrier and protocol, and `bondy_router.http_carriers` maps that carrier to the module serving it.
* Two HTTP status mappings changed, resolving a disagreement between the two tables that previously existed. `wamp.error.not_authorized` (the router refusing a peer) now maps to `403`, not `401`; `wamp.error.authorization_failed` (the router unable to determine whether an operation is permitted) now maps to `500`, not `403`. Responses to failed *authentication* still carry `401` with a `WWW-Authenticate` header, which is unchanged.
* An API Gateway specification's `status_codes` map now overrides the built-in defaults. Previously the defaults were merged over it at the host level, so a colliding operator-supplied entry was silently ignored — while the same map at the version level did take effect.
* `bondy_connect_sdk` returned `wamp.error.internal_error` when a callee handler crashed. The WAMP specification does not define that URI and reserves the `wamp.` namespace; it is now `bondy.error.internal_error`.
* The meta API reports an inconsistency as `bondy.error.inconsistency_error` rather than the generic `wamp.error.invalid_argument`. The payload's `code` is unchanged.
* The HTTP API Gateway request identifier — available to specifications as `{{request.id}}` and emitted as the `id` log metadata field — is now 32 lowercase hex characters (a W3C Trace Context `trace-id`) rather than a decimal integer.
* The router OTP application was renamed `bondy` → `bondy_router` (directory `apps/bondy_router`). The release, node name, configuration file and WAMP URIs are unchanged (`bondy`). Any `advanced.config` stanza keyed by the old application name (`{bondy, [...]}`) must move to `{bondy_router, [...]}` — the old key is silently inert, not an error.
* The experimental RPC Gateway application was renamed `bondy_rpc_gateway` → `bondy_http_connector` (directory `apps/bondy_http_connector`), to stop it being confused with the unrelated, inbound HTTP API Gateway: the established convention going forward is `bondy_<protocol>_gateway` for ingress, `bondy_<protocol>_connector` for egress. Every `rpc_gateway.*` `bondy.conf` key is now `http_connector.*` — see the [configuration migration guide](doc/guides/configuration/migrating_from_1.0.0-rc.65.md) for the full mapping.
* On-disk data from previous releases (PlumDB/RocksDB) is not readable by this release; there is no in-place migration, and no tool converts a PlumDB backup into this release's format (`bondy_export`, this release's data mover, reads and writes only the new storage engine). Recreate realms, users, grants and other security state on the new cluster — replay the same provisioning calls or scripts you used originally — and wipe stale data directories before starting the new release.
* The PlumDB `store.*` configuration options were removed, together with the `plum_db` broadcast wiring. New storage options live under `db.*` — see the [configuration migration guide](doc/guides/configuration/migrating_from_1.0.0-rc.65.md) for the full old-key → new-key mapping and the [reclamation configuration reference](doc/guides/configuration/reclamation_options.md) for the reclamation/retirement options.
* The `bondy_router.oplog_core_scan_max_concurrency` / `oplog.core.scan_max_concurrency` setting is removed — it had no effect in any released version.
* Requests with more than 100 query-string parameters or form fields are now rejected (Cowboy 2.17 default).
* **The fixed per-scheme listener configuration was removed.** `admin_api.{http,https}.*`, `api_gateway.{http,https}.*`, `wamp.{tcp,tls}.*` and `bridge.listener.{tcp,tls}.*` no longer exist; listeners are declared under `listeners.$name.*` (above). Two consequences are worth stating plainly, because a `bondy.conf` that still uses the old keys boots without complaint: a listener that is not declared **does not exist** — a file with no `listeners.*` key at all starts three built-in defaults (`admin`, `api_gateway_http`, `wamp_tcp`) and nothing else — and renaming the option keys alone is not sufficient, since a listener with options but no `transport`, `protocol` and bind target is refused at boot. Every enabled `transport = tls` listener must now state its own certificate: the removed keys supplied a default path and nothing does now. A disabled listener is not checked, so its certificate can be provisioned later. `scripts/migrate_conf.escript` reports both failures and applies the renames; the full mapping, including where the tails move, is in the [listeners guide](doc/guides/configuration/listeners.md#migrating-from-the-pre-10-keys) and step 8 of the [configuration migration guide](doc/guides/configuration/migrating_from_1.0.0-rc.65.md).
* `listeners.$name.ip` takes a literal IP address. The removed `*.ip` keys accepted any resolvable hostname; a name is now refused while the configuration file is read. An inventory supplied through `sys.config` is still resolved by name.
...
* A new option `serializers.json.float_format` has been added to `bondy.conf` that takes a string representation of the options supported by `erlang:float_to_binary/2`.
