...
* New configuration flag `wamp.dealer.progressive_call_results` (default `off`). Only enable it once every node in the cluster has been upgraded: a node without support settles a call on its first progressive result.
...
* New configuration flag `wamp.dealer.progressive_calls` (default `off`). Only enable it once every node in the cluster has been upgraded.
