...
* Sharding and physical layout are configurable per table: realm-partitioned or key-hashed placement, configurable shard counts (`db.core.shard_count`, `db.core.partition_strategy`, `db.core.realm_prefix_depth`), and aggregate-root routing that co-locates related keys (e.g. a user's group memberships) on a single shard for the authentication hot path. Realm identifiers are folded into storage keys so identically-named keys in different realms can never collide.
