...
-module(bondy_rest_security_handler).
...
init(Req, Opts) ->
    Ctxt = bondy_context:set_peer(bondy_context:new(), cowboy_req:peer(Req)),
    St = #state{
        context = Ctxt,
...
lookup(user, RealmUri, Id) ->
    bondy_security_user:lookup(RealmUri, Id);

lookup(group, RealmUri, Id) ->
    bondy_security_group:lookup(RealmUri, Id).


%% @private
delete(user, State) ->
    
    bondy_security:del_user(realm_uri(State), id(State));

delete(group, State) ->
    bondy_security:del_group(realm_uri(State), id(State));

delete(source, #state{master = user} = _State) ->
    %% TODO Implement, we need to receive the CIDR
    % bondy_security_user:remove_source(realm_uri(State), id(State), CIDR).
    error(not_implemented).


list(user, State) ->
    bondy_security_user:list(realm_uri(State));

list(group, State) ->
    bondy_security_group:list(realm_uri(State));

list(source, State) ->
    bondy_security_source:list(realm_uri(State)).


% add_user(Username, Password, Info) ->
%     case bondy_security:add_user(Username, )
