...
-define(LIMIT, 1000).
-define(POOL_NAME, juno_broker_pool).
...
handle_message(#subscribe{} = M, Ctxt) ->
    ...
    Reply = juno_message:subscribed(ReqId, SubsId),
    {reply, Reply, Ctxt};
...
subscribe(TopicUri, Options, Ctxt) ->
    ...
    case ets:match(Tab, SS0) of
        [[SubsId]] ->
            ...
            {ok, SubsId};
        [] ->
            SubsId = juno_id:new({session, SessionId}),
...
%% @end.
%% -----------------------------------------------------------------------------
-spec cast(term(), juno_context:context()) -> ok | overload | {error, any()}.
cast(M, Ctxt) ->
    PoolName = pool_name(),
...
handle_event({#publish{} = M, Ctxt}, _State) ->
    ...
    case publish(TopicUri, Opts, Args, Payload, Ctxt) of
        {ok, PubId} when Acknowledge == true ->
            Reply = juno_message:published(ReqId, PubId),
            To = juno_session:pid(juno_context:session(Ctxt)),
            juno:send(Reply, To, Ctxt),
            %% TODO publish metaevent
            ok;
        {ok, _} ->
            %% TODO publish metaevent
...
handle_event({#unsubscribe{} = M, Ctxt}, _State) ->
    ...
    Reply = case unsubscribe(SubsId, Ctxt) of
        ok ->
            juno_message:unsubscribed(ReqId);
        {error, no_such_subscription} ->
            juno_error:error(
