...
encode_term(Term, #wamp_erl{level = Level}) when
    is_atom(Term), Term =/= undefined, not is_boolean(Term), Level > 1
->
...
encode_term([{_, _} | _] = Term, Opts0) ->
    ...
    maps:from_list(
        lists:map(
            fun({K, V}) ->
                {encode_term(K, Opts), encode_term(V, Opts)}
            end,
            Term
        )
    );
...
parse_option([], Opt) ->
    Opt;
parse_option([{keys, K} | T], Opt) when
    K =:= binary; K =:= atom; K =:= existing_atom; K =:= attempt_atom
->
