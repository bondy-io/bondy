...
error(ReqType, ReqId, Details, ErrorUri, {Enc, Bin} = Partial) when
    is_atom(Enc), is_binary(Bin)
->
...
error(ReqType, ReqId, Details0, ErrorUri, Args0, KWArgs0) when
    is_map(Details0)
->
...
-spec subscriber_received(id(), map(), uri()) ->
    wamp_subscriber_received() | no_return().
...
set_partial(#call{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#call{partial = Partial};
set_partial(#error{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#error{partial = Partial};
set_partial(#event{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#event{partial = Partial};
set_partial(#invocation{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#invocation{partial = Partial};
set_partial(#publish{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#publish{partial = Partial};
set_partial(#result{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
    M#result{partial = Partial};
set_partial(#yield{} = M, {Enc, Bin} = Partial) when
    (Enc =:= json orelse Enc =:= cbor) andalso is_binary(Bin)
->
...
%% @private
validate_payload([Payload] = Args, undefined, #{ppt_scheme := _}) when
    is_binary(Payload)
->
    {Args, undefined};
