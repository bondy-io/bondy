...
do_on_update(RealmUri, Username, false) ->
    %% 1. Revoke all auth tickets. OAuth tokens are revoked off the
    %%    `[bondy, user, updated]` event below, asynchronously, together with a
    %%    second ticket sweep — see `bondy_event_wamp_publisher`.
