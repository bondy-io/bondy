...
new(Type, Target0, SessionId, Nodestring) when is_binary(Nodestring) ->
    is_binary(SessionId) orelse
        (SessionId == undefined andalso Type =/= client) orelse
        error({badarg, [{type, Type}, {session_id, SessionId}]}),

    lists:member(Type, ?TYPES) orelse
...
pattern(Type, Target0, SessionId, Node) ->
    ...
    lists:member(Type, ?TYPES ++ ['_']) orelse
        error({badarg, {type, Type}}),
    ...
    is_binary(SessionId) orelse
        SessionId == '_' orelse
        error({badarg, {session_id, SessionId}}),
...
validate_target(Type, Target, AllowPattern) ->
    case Target of
        ...
        {name, _} = Val ->
            Val;
        {callback, {M, F}} = Val ->
            (Type == internal orelse Type == '_') andalso
                is_atom(M) andalso
                is_atom(F) orelse
                badtarget(Type, Target),
