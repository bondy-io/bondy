...
format(
    Map = #{
        msg :=
            {report, #{
                label := {error_logger, _}, format := Format, args := Terms
            }}
    },
    UsrConfig
) ->
...
format(#{level := Level, msg := {report, Msg0}, meta := Meta}, UsrConfig) when
    is_map(Msg0)
->
...
format_log([{{msg, Key}, IfExists, Else} | Rest], Config, Msg, Meta, Acc) when
    is_atom(Key)
->
    case maps:take(Key, Msg) of
        error ->
...
format_val(colored_start, alert, #{colored := true, colored_alert := BOC}) ->
    BOC;
format_val(colored_start, emergency, #{
    colored := true, colored_emergency := BOC
}) ->
    BOC;
