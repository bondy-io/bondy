...
all_headers_enabled(_Config) ->
    ...
    ?assertEqual(
        <<"default-src 'self'">>,
        maps:get(<<"content-security-policy">>, Headers)
    ),
...
custom_values(_Config) ->
    ...
    Headers = bondy_http_security_headers:headers(Name),
    ?assertEqual(
        <<"max-age=600">>, maps:get(<<"strict-transport-security">>, Headers)
    ),
    ?assertEqual(<<"DENY">>, maps:get(<<"x-frame-options">>, Headers)),
    ?assertEqual(
        <<"script-src 'none'">>,
...
server_header_bondy(_Config) ->
    ...
    mock_config(
        Name, bondy_http_security_headers:default_config(), <<"bondy">>
    ),
    ok = bondy_http_security_headers:init(Name),
    Headers = bondy_http_security_headers:headers(Name),
...
server_header_custom(_Config) ->
    ...
    mock_config(
        Name, bondy_http_security_headers:default_config(), <<"my-server/1.0">>
    ),
    ok = bondy_http_security_headers:init(Name),
    Headers = bondy_http_security_headers:headers(Name),
...
mock_config(ListenerName, SecurityConfig, ServerHeader) ->
    meck:expect(
        ...
        get,
        fun
            ([N, security_headers], _Default) when N =:= ListenerName ->
                SecurityConfig;
