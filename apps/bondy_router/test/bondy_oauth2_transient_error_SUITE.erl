...
%% cowboy would exchange. A successful grant is `{true, Req, St}` with the
%% token in `Req`'s response body — cowboy_rest, not the handler, sends
%% that 200 — so it is read off the returned request. Returns
%% `{Status, Headers, Body}` either way.
post_token(ClientId, Password, Body) ->
    ...
    Handler = spawn_link(fun() ->
        ...
        {true, Req1, St1} = bondy_oauth2_rest_handler:is_authorized(Req0, St0),
        Parent ! {accepted, self(), bondy_oauth2_rest_handler:accept(Req1, St1)}
    end),
    serve_body(Handler, Body),
    receive
...
serve_body(Handler, Body) ->
    receive
        {{_Conn, 1}, {read_body, Handler, Ref, _Length, _Period}} ->
            ...
            ok
    after 5000 ->
        exit(handler_never_read_the_body)
    end.
