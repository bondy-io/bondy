...
-module(bondy_http_connector_manager).
...
4. groups service procedures by realm and spawns a `bondy_http_connector_callee`
   per service/realm.
...
-doc false.
init([]) ->
    Services = application:get_env(bondy_http_connector, services, []),
...
handle_continue(start_callees, State) ->
    lists:foreach(
        fun({RealmUri, Service, PoolName}) ->
            case
                bondy_http_connector_callee_sup:start_callee(
            ...
            of
                ...
                {error, Reason} ->
                    ?LOG_ERROR(#{
                        description => "Failed to start HTTP Connector callee",
...
resolve_service_secret(
    ...
    Result = bondy_http_connector_secret_resolver:resolve_service_secrets(
        SecretsSpec, Name
    ),
...
start_http_pools(Services) ->
    Map = lists:foldl(
        fun(#{name := ServiceName} = ServiceConf, Acc) ->
            ...
            %% `service_name` is the human-readable name (the pool's own
            %% `Name` arg is the mangled atom `..._http_pool_<service>`,
            %% not fit for a Prometheus label); `liveness` carries the
            %% periodic up-state health-check config (schema
            %% `http_connector.services.$service.liveness.*`).
            PoolOpts = PoolOpts0#{
                ssl_options => SslOpts,
                service_name => ServiceName,
                liveness => maps:get(liveness, ServiceConf, #{})
            },
            case
                bondy_http_connector_http_pool_sup:start_pool(
