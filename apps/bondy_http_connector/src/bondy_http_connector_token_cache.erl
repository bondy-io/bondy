...
-module(bondy_http_connector_token_cache).

-moduledoc """
Public API for the per-service token cache.

A pool of `bondy_http_connector_token_cache_worker` processes is started
...
├── bondy_http_connector_token_cache        ← this module (registry)
└── bondy_http_connector_token_cache_sup    (one_for_one)
    ├── worker_1
    ├── worker_2
    └── ...
```
...
{ok, Token} = bondy_http_connector_token_cache:get(
    <<"service_a">>, bondy_http_connector_auth_generic, AuthConf
).

%% Invalidate after upstream 401
ok = bondy_http_connector_token_cache:invalidate(<<"service_a">>).

%% Next get will fetch a fresh token
{ok, FreshToken} = bondy_http_connector_token_cache:get(
    <<"service_a">>, bondy_http_connector_auth_generic, AuthConf
).
```
...
Called by `bondy_http_connector_sup`. Creates the ETS table used for
...
{ok, Token} = bondy_http_connector_token_cache:get(
    ~"service_b", bondy_http_connector_auth_generic, AuthConf
).
```
""".
    ...
    do_for_worker(
        fun
            (noproc) ->
                {error, noproc};
            (WorkerName) ->
                bondy_http_connector_token_cache_worker:get_token(
...
init_pool() ->
    ...
    [
        begin
            ...
            Result = bondy_http_connector_token_cache_sup:start_worker(
                ?POOLNAME, WorkerName
            ),
            resulto:try_recover(
                Result,
...
worker_name(Id) when is_integer(Id) ->
    list_to_atom(
        "bondy_http_connector_token_cache_worker_" ++ integer_to_list(Id)
    ).
