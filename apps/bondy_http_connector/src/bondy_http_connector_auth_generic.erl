...
-module(bondy_http_connector_auth_generic).
...
{ok, {<<"eyJ...">>, #{expires_in => 3600}}} = bondy_http_connector_auth_generic:fetch_token(Conf).
```
""".
...
{Url, [{<<"Authorization">>, <<"Bearer abc">>}]} =
    bondy_http_connector_auth_generic:apply_auth(<<"abc">>, Url, [], Conf).
```
""".
