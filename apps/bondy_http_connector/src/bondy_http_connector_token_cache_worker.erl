...
-module(bondy_http_connector_token_cache_worker).

-moduledoc """
Per-service token worker.

Each instance manages exactly one service's authentication token.
Workers are created on demand by `bondy_http_connector_token_cache` and
supervised under `bondy_http_connector_token_cache_sup` as `temporary`
children.
...
-type t() :: #state{}.
-type http_connector_token() :: #http_connector_token{}.

-export_type([http_connector_token/0]).
...
->
    Default = undefined,
    Pos = #http_connector_token.token,

    case ets:lookup_element(WorkerName, ServiceName, Pos, Default) of
        undefined ->
            ok = bondy_http_connector_telemetry:token_cache(ServiceName, miss),
            Cmd = {get_token, ServiceName, AuthMod, AuthConf},
            gen_server:call(WorkerName, Cmd);
        Token ->
            ok = bondy_http_connector_telemetry:token_cache(ServiceName, hit),
            {ok, Token}
    end.
...
handle_call({get_token, ServiceName, AuthMod, AuthConf}, _From, State) ->
    ...
    case ets:lookup(WorkerName, ServiceName) of
        [#http_connector_token{token = Token, expires_at = ExpiresAt}] when
        ...
        Ref ->
            ...
            ets:update_element(
                State#state.name, ServiceName, [
                    {#http_connector_token.token, undefined},
                    {#http_connector_token.expires_at, undefined},
                    {#http_connector_token.timer_ref, undefined}
                ]
            ),
            ok
    end.
