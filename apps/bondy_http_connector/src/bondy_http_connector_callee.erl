...
-module(bondy_http_connector_callee).

-moduledoc """
A WAMP Callee that registers procedures for a single service/realm pair.

Spawned by `bondy_http_connector_callee_sup` (one instance per service per
...
to `bondy_http_connector_callee_handler:handle_wamp_call`.
...
```erlang
MF = {bondy_http_connector_callee_handler, handle_wamp_call},
...
-record(state, {
    ...
    poolname :: atom(),
    registrations :: #{
        binary() => {term(), binary(), #http_connector_proc_conf{}}
    }
}).
...
init([RealmUri, Service, PoolName]) ->
    ...
    SessionOpts = #{
        type => internal,
        roles => #{callee => #{}},
        agent => <<"bondy_http_connector_callee">>,
        is_anonymous => true
    },
            ...
            FullProcConf =
                case SecretVars of
                    not_ready ->
                        Base;
                    _ ->
                        bondy_http_connector_callee_handler:apply_secrets(
                            Base, SecretVars
                        )
                end,
...
%% for delegating to `bondy_http_connector_callee_handler:apply_secrets/2`
...
build_proc_conf(
    ...
    PathVars = bondy_http_connector_callee_handler:extract_path_vars(Path),
