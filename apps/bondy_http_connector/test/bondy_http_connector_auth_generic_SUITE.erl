-module(bondy_http_connector_auth_generic_SUITE).

-moduledoc """
Tests for `bondy_http_connector_auth_generic:apply_auth/4`.
...
apply_auth_query_param(_Config) ->
    ...
    {Url, Headers} = bondy_http_connector_auth_generic:apply_auth(
...
apply_auth_query_param_existing_qs(_Config) ->
    Conf = #{apply => #{placement => query_param, name => <<"token">>}},
    {Url, _} = bondy_http_connector_auth_generic:apply_auth(
...
apply_auth_header_plain(_Config) ->
    ...
    {_, Headers} = bondy_http_connector_auth_generic:apply_auth(
