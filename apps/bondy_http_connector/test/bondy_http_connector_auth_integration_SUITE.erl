-module(bondy_http_connector_auth_integration_SUITE).

-moduledoc """
Integration tests for `bondy_http_connector_auth_generic` against
...
### OAuth2 config equivalent (bondy.conf)
    ...
    http_connector.services.my_api.auth.fetch.method = post
    http_connector.services.my_api.auth.fetch.url = {{token_endpoint}}
    http_connector.services.my_api.auth.fetch.body_encoding = form
    http_connector.services.my_api.auth.fetch.body.grant_type = client_credentials
    http_connector.services.my_api.auth.fetch.token_path = access_token
    http_connector.services.my_api.auth.fetch.expires_in_path = expires_in
    http_connector.services.my_api.auth.fetch.basic_auth.username = {{client_id}}
    http_connector.services.my_api.auth.fetch.basic_auth.password = {{client_secret}}

    http_connector.services.my_api.auth.apply.placement = header
    http_connector.services.my_api.auth.apply.name = Authorization
    http_connector.services.my_api.auth.apply.format = Bearer {{token}}

    http_connector.services.my_api.auth.vars.token_endpoint = https://idp.example.com/token
    http_connector.services.my_api.auth.vars.client_id = my-app
    http_connector.services.my_api.auth.vars.client_secret = s3cret

### Custom token config equivalent (bondy.conf)

    http_connector.services.gis.auth.fetch.method = post
    http_connector.services.gis.auth.fetch.url = {{base_url}}/portal/sharing/rest/generateToken
    http_connector.services.gis.auth.fetch.body_encoding = form
    http_connector.services.gis.auth.fetch.body.username = {{username}}
    http_connector.services.gis.auth.fetch.body.password = {{password}}
    http_connector.services.gis.auth.fetch.body.f = json
    http_connector.services.gis.auth.fetch.body.referer = https://www.example.com
    http_connector.services.gis.auth.fetch.token_path = token
    http_connector.services.gis.auth.fetch.error_path = error.message

    http_connector.services.gis.auth.apply.placement = query_param
    http_connector.services.gis.auth.apply.name = token

    http_connector.services.gis.auth.vars.base_url = https://gis.example.com
    http_connector.services.gis.auth.vars.username = admin
    http_connector.services.gis.auth.vars.password = hunter2
""".
...
cc_fetch_token(Config) ->
    ...
    {ok, {Token, _Meta}} = bondy_http_connector_auth_generic:fetch_token(Conf),
...
cc_fetch_token_with_expires_in(Config) ->
    ...
    Conf = cc_config(Config),
    {ok, {Token, Meta}} = bondy_http_connector_auth_generic:fetch_token(Conf),
...
cc_apply_auth_bearer_header(_Config) ->
    ...
    {ResultUrl, ResultHeaders} =
        bondy_http_connector_auth_generic:apply_auth(
            <<"eyJabc123">>, Url, [], Conf
        ),
...
cc_fetch_basic_auth_sent(Config) ->
    ...
    {ok, {_Token, _Meta}} = bondy_http_connector_auth_generic:fetch_token(Conf),
...
cc_fetch_invalid_credentials(Config) ->
    ...
    Result = bondy_http_connector_auth_generic:fetch_token(Conf),
...
gis_fetch_token(Config) ->
    ...
    {ok, Token} = bondy_http_connector_auth_generic:fetch_token(Conf),
