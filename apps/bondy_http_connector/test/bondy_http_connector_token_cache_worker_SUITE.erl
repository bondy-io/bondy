-module(bondy_http_connector_token_cache_worker_SUITE).

-moduledoc """
Unit tests for `bondy_http_connector_token_cache_worker`.
...
-define(POOL_NAME, bondy_http_connector_token_cache_worker_test_pool).
-define(WORKER_NAME, bondy_http_connector_token_cache_worker_test_w).
...
start_worker() ->
    {ok, Pid} = bondy_http_connector_token_cache_worker:start_link(
...
get_token(Pid, Conf) ->
    gen_server:call(
        Pid,
        {get_token, ?SERVICE_NAME, bondy_http_connector_mock_auth, Conf}
    ).
...
get_token_fetches_on_cold_start(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"fresh-tok">>),
    Pid = start_worker(),
...
get_token_caches_on_hot(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"cached-tok">>),
    Pid = start_worker(),
    {ok, _} = get_token(Pid),
    {ok, T2} = get_token(Pid),
    {ok, T3} = get_token(Pid),
...
invalidate_clears_cached_token(_Config) ->
    ...
    bondy_http_connector_mock_auth:set_fetch_fun(fun(_) ->
...
fetch_error_returns_error(_Config) ->
    bondy_http_connector_mock_auth:set_error(boom),
    Pid = start_worker(),
...
preemptive_refresh_failure_keeps_old_token(_Config) ->
    ...
    bondy_http_connector_mock_auth:set_error(refresh_failed),
    timer:sleep(1500),
