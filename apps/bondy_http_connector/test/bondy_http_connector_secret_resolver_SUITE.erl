-module(bondy_http_connector_secret_resolver_SUITE).

-moduledoc """
Tests for bondy_http_connector_secret_resolver.
...
->
    %% Stub mocks (NOT passthrough): the resolver only ever calls
    %% `erlcloud_aws:auto_config/0`, `erlcloud_aws:service_config/3` and
    %% `erlcloud_sm:get_secret_value/3`, so mocking exactly those is enough.
    %% `[passthrough]` recompiles the whole (large) erlcloud modules to forward
    %% unmocked calls — and when this suite runs after another suite has started
    %% bondy (which loads erlcloud), that recompile blocks the meck server long
    %% enough for the CT watchdog to kill the case. Stub mocks need no recompile,
    %% so they are immune to that ordering (the suite passed in isolation only
    %% because nothing had loaded erlcloud yet). `no_link` keeps a meck-server
    %% fault from cross-killing the test process.
...
decode_basic_auth_with_prefix(_Config) ->
    ...
    ?assertEqual(
        {<<"user">>, <<"pass">>},
        bondy_http_connector_secret_resolver:decode_basic_auth(Header)
    ).
...
decode_basic_auth_without_prefix(_Config) ->
    ...
    ?assertEqual(
        {<<"user">>, <<"pass">>},
        bondy_http_connector_secret_resolver:decode_basic_auth(Encoded)
    ).
...
transform_none(_Config) ->
    ?assertEqual(
        <<"hello">>,
        bondy_http_connector_secret_resolver:apply_transform(<<"hello">>, none)
    ).
...
transform_basic_username(_Config) ->
    ...
    ?assertEqual(
        <<"alice">>,
        bondy_http_connector_secret_resolver:apply_transform(
...
resolve_services_with_secrets(_Config) ->
    ...
    [Resolved] = bondy_http_connector_secret_resolver:resolve_services([Service]),
...
resolve_services_missing_field(_Config) ->
    ...
    ?assertError(
        {secret_field_not_found, <<"missing-svc">>, my_var,
            <<"MISSING_FIELD">>},
        bondy_http_connector_secret_resolver:resolve_services([Service])
    ).
...
resolve_service_secrets_ok(_Config) ->
    ...
    {ok, Vars} = bondy_http_connector_secret_resolver:resolve_service_secrets(
        SecretsSpec, <<"ok-svc">>
    ),
    ?assertEqual(
        <<"https://idp.example.com/token">>,
...
resolve_service_secrets_aws_error(_Config) ->
    ...
    {error, Reason} =
        bondy_http_connector_secret_resolver:resolve_service_secrets(
            SecretsSpec, <<"err-svc">>
        ),
