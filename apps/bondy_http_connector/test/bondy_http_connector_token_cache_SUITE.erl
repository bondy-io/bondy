...
init_per_testcase(_TC, Config) ->
    ...
    %% not the full bondy_http_connector_sup which requires service config.
...
token_fetch_total(Service, Outcome) ->
    metric_or_zero(bondy_http_connector_token_fetch_total, #{
...
cold_cache_fetches_token(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"tok-1">>),
    {ok, Token} = bondy_http_connector_token_cache:get(
        <<"svc-a">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    ?assertEqual(<<"tok-1">>, Token),
    ?assertEqual(1, bondy_http_connector_mock_auth:call_count()),
    ?assertEqual(1, token_cache_total(<<"svc-a">>, miss)),
    ?assertEqual(0, token_cache_total(<<"svc-a">>, hit)),
    ?assertEqual(1, token_fetch_total(<<"svc-a">>, ok)).

hot_cache_returns_same_token(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"tok-2">>),
    {ok, T1} = bondy_http_connector_token_cache:get(
        <<"svc-b">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    {ok, T2} = bondy_http_connector_token_cache:get(
        <<"svc-b">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    ?assertEqual(T1, T2),
    ?assertEqual(<<"tok-2">>, T1),
    ?assertEqual(1, token_cache_total(<<"svc-b">>, miss)),
    ?assertEqual(1, token_cache_total(<<"svc-b">>, hit)).

hot_cache_no_extra_fetch(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"tok-3">>),
    _ = bondy_http_connector_token_cache:get(
        <<"svc-c">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    _ = bondy_http_connector_token_cache:get(
        <<"svc-c">>, bondy_http_connector_mock_auth, auth_conf()
    ),
...
invalidate_forces_fresh_fetch(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"old-tok">>),
    {ok, <<"old-tok">>} = bondy_http_connector_token_cache:get(
        <<"svc-d">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    ?assertEqual(1, bondy_http_connector_mock_auth:call_count()),

    ok = bondy_http_connector_token_cache:invalidate(<<"svc-d">>),
    %% Give the cast time to process
    timer:sleep(10),

    bondy_http_connector_mock_auth:set_token(<<"new-tok">>),
    {ok, Token} = bondy_http_connector_token_cache:get(
        <<"svc-d">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    ?assertEqual(<<"new-tok">>, Token),
    ?assertEqual(2, bondy_http_connector_mock_auth:call_count()).

invalidate_nonexistent_service_ok(_Config) ->
    %% Should not crash
    ?assertEqual(
        ok, bondy_http_connector_token_cache:invalidate(<<"no-such-svc">>)
    ).

multiple_services_isolated(_Config) ->
    Counter = atomics:new(1, [{signed, false}]),
    bondy_http_connector_mock_auth:set_fetch_fun(fun(_Conf) ->
        N = atomics:add_get(Counter, 1, 1),
        {ok, <<"tok-", (integer_to_binary(N))/binary>>}
    end),

    {ok, TA} = bondy_http_connector_token_cache:get(
        <<"iso-a">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    {ok, TB} = bondy_http_connector_token_cache:get(
        <<"iso-b">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    %% Each service got its own token
    ?assertNotEqual(TA, TB),

    %% Invalidating one does not affect the other
    ok = bondy_http_connector_token_cache:invalidate(<<"iso-a">>),
    timer:sleep(10),
    {ok, TB2} = bondy_http_connector_token_cache:get(
        <<"iso-b">>, bondy_http_connector_mock_auth, auth_conf()
    ),
    ?assertEqual(TB, TB2).

worker_crash_cleans_registry(_Config) ->
    bondy_http_connector_mock_auth:set_token(<<"crash-tok">>),
    {ok, _} = bondy_http_connector_token_cache:get(
        <<"crash-svc">>, bondy_http_connector_mock_auth, auth_conf()
    ),

    %% Find the worker pid via the pool and kill it
    PoolName = {bondy_http_connector_token_cache, pool},
    ...
    bondy_http_connector_mock_auth:set_token(<<"new-crash-tok">>),
    {ok, Token} = bondy_http_connector_token_cache:get(
        <<"crash-svc">>, bondy_http_connector_mock_auth, auth_conf()
    ),
...
concurrent_cold_cache_single_fetch(_Config) ->
    ...
    Pids = [
        spawn_link(fun() ->
            Result = bondy_http_connector_token_cache:get(
                <<"conc-svc">>, bondy_http_connector_mock_auth, auth_conf()
            ),
...
preemptive_refresh_fires(_Config) ->
    ...
    {ok, T1} = bondy_http_connector_token_cache:get(
        <<"refresh-svc">>, bondy_http_connector_mock_auth, Conf
    ),
...
fetch_error_propagates(_Config) ->
    bondy_http_connector_mock_auth:set_error(endpoint_unreachable),
    Result = bondy_http_connector_token_cache:get(
        <<"err-svc">>, bondy_http_connector_mock_auth, auth_conf()
    ),
...
meta_expires_in_respected(_Config) ->
    ...
    {ok, <<"meta-tok-1">>} = bondy_http_connector_token_cache:get(
        <<"meta-svc">>, bondy_http_connector_mock_auth, Conf
    ),
