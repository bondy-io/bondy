-module(bondy_http_connector_callee_handler_SUITE).

-moduledoc """
Tests for `bondy_http_connector_callee_handler`.
...
path_no_vars(_Config) ->
    KWArgs = #{<<"foo">> => <<"bar">>},
    {<<"/invoices">>, KWArgs} =
        bondy_http_connector_callee_handler:interpolate_path(
            <<"/invoices">>, KWArgs
        ).
...
extract_vars_empty(_Config) ->
    ?assertEqual(
        [],
        bondy_http_connector_callee_handler:extract_path_vars(<<"/plain/path">>)
    ).
...
extract_vars_multiple(_Config) ->
    Vars = bondy_http_connector_callee_handler:extract_path_vars(
...
route_get_empty_kwargs(_Config) ->
    {Url, Body} = bondy_http_connector_callee_handler:route_kwargs(
...
success_200_json(_Config) ->
    ...
    {ok, Details, Args, KWArgs} =
        bondy_http_connector_callee_handler:http_to_wamp(200, JsonBody),
    ?assertEqual(#{}, Details),
    ?assertEqual([], Args),
...
success_201_json(_Config) ->
    ...
    {ok, _, _, KWArgs} =
        bondy_http_connector_callee_handler:http_to_wamp(201, JsonBody),
...
error_response_includes_status_and_body(_Config) ->
    ...
    {error, _Uri, _Details, _Args, KWArgs} =
        bondy_http_connector_callee_handler:http_to_wamp(404, JsonBody),
...
handle_call_pending_service(_Config) ->
    ...
    ets:insert(bondy_http_connector_manager, {ServiceName, not_ready}),
    ?assertEqual(
        {error, not_ready},
...
handle_call_ready_service(_Config) ->
    ...
    ets:insert(
        bondy_http_connector_manager, {ServiceName, {ready, ExtraVars}}
    ),

    {ok, Vars} = bondy_http_connector_manager:service_readiness(ServiceName),
