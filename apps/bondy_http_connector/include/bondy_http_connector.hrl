...
-ifndef(BONDY_HTTP_CONNECTOR_HRL).
-define(BONDY_HTTP_CONNECTOR_HRL, true).

%% Per-call defaults applied when a procedure config does not override them.
%% Schema-level defaults live in schema/bondy_http_connector.schema; these are
...
%% by `bondy_http_connector_callee:register_procedures/1` and passed to the WAMP
%% dealer as the callback args, so `bondy_http_connector_callee_handler` receives
...
%% the `bondy_http_connector_manager` moduledoc.
-record(http_connector_proc_conf, {
    service_name :: binary(),
    base_url :: binary(),
    auth_mod :: module(),
    auth_conf :: map(),
