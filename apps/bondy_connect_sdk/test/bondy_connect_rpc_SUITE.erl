...
Bondy router over raw TCP: a `bondy_connect_sdk` callee registers a procedure whose
...
register_and_call(_) ->
    ...
    {ok, RegId} = bondy_connect_client:register(
        Callee, <<"com.example.echo">>, Echo
    ),
    ?assert(is_integer(RegId)),

    Caller = connect(),
    {ok, Result} = bondy_connect_client:call(
...
call_async_token_reply(_) ->
    ...
    {ok, _} = bondy_connect_client:register(
        Callee, <<"com.example.async">>, Echo
    ),

    Caller = connect(),
    {ok, Token} = bondy_connect_client:call_async(
        Caller, <<"com.example.async">>, [
            <<"x">>
        ]
    ),
...
unregister_stops_routing(_) ->
    ...
    Caller = connect(),
    Result = bondy_connect_client:call(Caller, <<"com.example.transient">>, []),
    ?assertMatch(
...
handler_error_propagates(_) ->
    ...
    {ok, _} = bondy_connect_client:register(
        Callee, <<"com.example.err">>, Failing
    ),

    Caller = connect(),
    Result = bondy_connect_client:call(Caller, <<"com.example.err">>, []),
    ?assertMatch(
        {error, #{kind := wamp, uri := <<"com.example.business_error">>}},
        Result
    ),
...
handler_crash_isolation(_) ->
    ...
    {ok, _} = bondy_connect_client:register(
        Callee, <<"com.example.crash">>, Boom
    ),

    Caller = connect(),
    Result = bondy_connect_client:call(Caller, <<"com.example.crash">>, []),
    ?assertMatch(
    ...
    {ok, _} = bondy_connect_client:register(
        Callee, <<"com.example.still_ok">>, Ok
    ),
    {ok, R2} = bondy_connect_client:call(Caller, <<"com.example.still_ok">>, [
        <<"alive">>
    ]),
...
per_call_timeout(_) ->
    ...
    {ok, _} = bondy_connect_client:register(
        Callee, <<"com.example.slow">>, Slow
    ),

    Caller = connect(),
    Result = bondy_connect_client:call(
...
load_rejection_under_burst(_) ->
    ...
    {ok, Callee} = bondy_connect_client:connect(
    ...
    {ok, _} = bondy_connect_client:register(Callee, Proc, Slow),
    ...
    Tokens = [
        begin
            {ok, T} = bondy_connect_client:call_async(Caller, Proc, []),
            T
        end
     || _ <- lists:seq(1, 3)
    ],
...
progressive_input_caller_unsupported(_) ->
    ...
    {ok, Caller} = bondy_connect_client:connect(
        spec(#{
            roles => #{
                caller => #{
                    features => #{
