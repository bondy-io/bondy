...
`bondy_connect_sdk` clients running in the CT controller VM — the caller
...
init_per_suite(Config) ->
    Nodes = bondy_ct:start_cluster(
        [
            {bcx_rpc1, [
                {[bondy_router, listeners], peer_listeners(?NODE1_WAMP_PORT)}
            ]},
            {bcx_rpc2, [
                {[bondy_router, listeners], peer_listeners(?NODE2_WAMP_PORT)}
            ]}
        ],
        Config
    ),
...
%% The client listener each peer needs, declared in the INVENTORY rather than by
%% re-enabling the legacy `bondy_router.wamp_tcp.*` keys.
%%
%% `bondy_ct:node_env/2` sets `[bondy_router, listeners]` for every cluster peer,
%% which puts the peer on the configured listener path — and on that path only
%% declared listeners exist, so a `wamp_tcp.enabled = true` written in the old
%% spelling creates nothing and the peer answers `econnrefused` on the port
%% below. Those keys no longer exist at all, which makes this a historical note
%% rather than a live hazard. Verified directly: with the legacy keys, all nine cases in
%% this suite fail with `{connect_failed, _, {shutdown, {connect_error,
%% econnrefused}}}`.
%%
%% The name is kept as `wamp_tcp` so the peer still inherits that block's tuning
%% from `bondy_ct`'s `?ENV` — `ping`, `proxy_protocol`, and the stream-socket
%% options — since a listener's option block is read at `[Name, ...]`. The bind
%% target comes from the spec, which `bondy_listener_ranch:with_bind/2` keystores
%% over the block's own `socket_opts.port`.
%%
%% `admin` is restated at `port => 0` because setting `[bondy_router, listeners]`
%% REPLACES what `node_env/2` installed: without it `with_reserved/1` injects
%% `admin` at its fixed default 18081 and both peers on this host race for it.
peer_listeners(WampPort) ->
    [
        {admin, #{
            transport => tcp,
            protocol => http,
...
progressive_remote_cancel_interrupts_callee(Config) ->
    ...
    ok = bondy_connect_client:cancel(Caller, Token, killnowait),
...
progressive_input_across_cluster(Config) ->
    ...
    Caller = connect(?NODE1_WAMP_PORT),
    {ok, Token} = bondy_connect_client:call_stream(Caller, Proc, [1], #{}, #{}),
    _ = [
...
progressive_input_remote_caller_death_interrupts_callee(Config) ->
    ...
    Caller = connect(?NODE1_WAMP_PORT),
    {ok, _Token} = bondy_connect_client:call_stream(
        Caller, Proc, [1], #{}, #{}
    ),
...
drain_progress(Token, Timeout) ->
    receive
        {bondy_connect_client, Token, {progress, _}} ->
            drain_progress(Token, Timeout);
        {bondy_connect_client, Token, Terminal} ->
            {ok, Terminal}
    after Timeout ->
