...
local_call_round_trip(_) ->
    ...
    {ok, R} = bondy_connect_client:call(Conn, <<"com.example.res.local">>, [
        <<"hi">>
    ]),
...
local_cross_session_call(_) ->
    ...
    {ok, _} = bondy_connect_client:register(Callee, Proc, fun(Args, _, _) ->
        {ok, #{args => [<<"from-callee">> | Args]}}
    end),
    {ok, R} = bondy_connect_client:call(Caller, Proc, [<<"x">>]),
