...
subscribe_and_receive(_) ->
    ...
    {ok, SubId} = bondy_connect_client:subscribe(
        Sub, <<"com.example.t1">>, Handler
    ),
    ?assert(is_integer(SubId)),

    Pub = connect(),
    ok = bondy_connect_client:publish(Pub, <<"com.example.t1">>, [<<"hi">>]),
    receive
...
acknowledged_publish(_) ->
    Pub = connect(),
    Result = bondy_connect_client:publish_ack(
...
ordered_events(_) ->
    ...
    {ok, _} = bondy_connect_client:subscribe(
        Sub, <<"com.example.ordered">>, Handler
    ),

    Pub = connect(),
    _ = [
        {ok, _} = bondy_connect_client:publish_ack(
...
subscriber_crash_preserves_fifo(_) ->
    ...
    ?assertEqual(established, bondy_connect_client:status(Sub)),
...
unsubscribe_stops_events(_) ->
    ...
    {ok, _} = bondy_connect_client:subscribe(
        Sub, <<"com.example.unsub.ctrl">>, Ctrl
    ),
    ok = bondy_connect_client:unsubscribe(Sub, SubId),
