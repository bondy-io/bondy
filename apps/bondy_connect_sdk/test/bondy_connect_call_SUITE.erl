...
-moduledoc """
M1 walking-skeleton integration tests: an anonymous `bondy_connect_sdk` client
...
named_connection(_) ->
    {ok, Conn} = bondy_connect_client:connect(m1_named, spec()),
    ...
    Named = bondy_connect_client:named(m1_named),
    ?assertEqual(established, bondy_connect_client:status(Named)),
    {ok, _} = bondy_connect_client:call(Named, <<"bondy.session.self">>, []),
    ok = bondy_connect_client:disconnect(Conn),
    %% Name is freed and can be reused.
    {ok, Conn2} = bondy_connect_client:connect(m1_named, spec()),
    ok = bondy_connect_client:disconnect(Conn2).

multiple_calls(_) ->
    {ok, Conn} = bondy_connect_client:connect(spec()),
    {ok, R1} = bondy_connect_client:call(Conn, <<"bondy.session.self">>, []),
    {ok, R2} = bondy_connect_client:call(Conn, <<"bondy.session.self">>, []),
...
call_unknown_procedure_errors(_) ->
    ...
    Result = bondy_connect_client:call(
        Conn, <<"com.example.no.such.proc.m1">>, []
    ),
...
connect_unknown_realm_fails(_) ->
    ...
    ?assertMatch({error, _}, bondy_connect_client:connect(Spec)).
...
call_after_disconnect_fails(_) ->
    ...
    ?assertMatch(
        {error, _},
        bondy_connect_client:call(Conn, <<"bondy.session.self">>, [])
    ).
