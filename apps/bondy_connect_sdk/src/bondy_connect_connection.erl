...
- **caller** — `call/5` (synchronous) and `call_async/5` (a token reply to the
  ...
  `{bondy_connect_client, Token, {progress, map()}}` before the terminal reply).
...
-spec call(pid(), uri(), list(), map(), map()) ->
    {ok, bondy_connect_client:call_result()}
    | {error, bondy_connect_client:call_error()}.
...
delivered to the calling process as `{bondy_connect_client, Token, Reply}` where
`Reply` is `{ok, bondy_connect_client:call_result()}` or
`{error, bondy_connect_client:call_error()}`.

With `receive_progress => true` any progressive results arrive first, each
as `{bondy_connect_client, Token, {progress, map()}}`; the terminal delivery
...
-spec call_async(pid(), uri(), list(), map(), map()) ->
    {ok, reference()} | {error, bondy_connect_client:call_error()}.
...
-spec register(pid(), uri(), bondy_connect_handler_spec:handler(), map()) ->
    {ok, pos_integer()} | {error, bondy_connect_client:call_error()}.
...
-spec unregister(pid(), pos_integer() | uri()) ->
    ok | {error, bondy_connect_client:call_error()}.
...
both `bondy_connect_client:publish/*` (acknowledge disallowed) and `publish_ack/5`
...
resolve_pending(ReqId, Reply, #data{pending = Pending} = Data) ->
    case maps:take(ReqId, Pending) of
        {#{from := From, timer := TRef} = Entry, Pending1} ->
            _ = cancel_timer(TRef),
            ok = notify_span(Entry, Reply, Data#data.config),
...
%% `error`, everything else as `success`. The configured `peer_service`
%% names the router on this CLIENT leg (the config map is validated,
%% so the key is always present).
notify_span(
    #{span := #{uri := Uri, trace := Trace, start := T0}}, Reply, Config
) ->
    ...
    bondy_connect_telemetry:rpc_latency(
        call, Uri, Duration, Trace, Outcome, maps:get(peer_service, Config)
    );
notify_span(_, _, _) ->
    ok.
...
notify_progress(ReqId, Payload, #data{pending = Pending} = Data) ->
    case maps:find(ReqId, Pending) of
        {ok,
            ...
            Pid ! {bondy_connect_client, Token, {progress, Payload}},
            _ = cancel_timer(TRef),
...
dispatch_reply({async, Pid, Token}, Reply) ->
    Pid ! {bondy_connect_client, Token, Reply},
    ok;
