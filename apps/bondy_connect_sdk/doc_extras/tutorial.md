...
# Tutorial — `bondy_connect_sdk`

A hands-on guide to the `bondy_connect_sdk` WAMP client: connecting, authenticating,
and using all four WAMP roles (**caller**, **callee**, **publisher**,
**subscriber**), plus resilience, load regulation, and the in-VM transport.

Every code snippet uses only the public API (the `m:bondy_connect_client` facade), so
you can paste it into a shell or a module and expect it to work against a
running Bondy router.
...
- **`connect/1,2` blocks until the session is established** (handshake + auth,
  ...
  `t:bondy_connect_client:conn/0`. Treat it as a token — pass it back to the API, never
  inspect it.
...
- **Standalone.** `bondy_connect_sdk` has **no dependency on the `bondy` router
...
The connection moves through these statuses, reported by `bondy_connect_client:status/1`:
...
`bondy_connect_sdk` is part of the Bondy umbrella. To embed it in an external
...
{deps, [
    {bondy_connect_sdk,
        {git_subdir, "https://github.com/bondy-io/bondy.git",
            {branch, "master"}, "apps/bondy_connect_sdk"}}
]}.
```

### Starting the application

`bondy_connect_sdk` is an OTP application; start it (and its dependencies) before
...
established = bondy_connect_client:status(Conn),
...
```erlang
{ok, _} = bondy_connect_client:connect(my_service_conn, #{
    transport => tcp,
    endpoint  => {"127.0.0.1", 18082},
    realm     => <<"com.example.realm">>
}),
...
Conn = bondy_connect_client:named(my_service_conn),
{ok, R} = bondy_connect_client:call(Conn, <<"com.example.echo">>, [<<"hi">>]).
```
...
```erlang
%% No args
{ok, R0} = bondy_connect_client:call(Conn, <<"bondy.session.self">>),

%% Positional args
{ok, R1} = bondy_connect_client:call(Conn, <<"com.example.add">>, [2, 3]),
[5] = maps:get(args, R1),

%% Positional + keyword args
{ok, R2} = bondy_connect_client:call(Conn, <<"com.example.greet">>, [<<"Alice">>], #{lang => <<"en">>}),

%% With options (per-call timeout in ms)
case bondy_connect_client:call(Conn, <<"com.example.slow">>, [], #{}, #{timeout => 5000}) of
...
```erlang
{ok, Token} = bondy_connect_client:call_async(Conn, <<"com.example.slow">>, [<<"job-1">>]),
receive
    {bondy_connect_client, Token, {ok, #{args := Args}}} -> {done, Args};
    {bondy_connect_client, Token, {error, Reason}}       -> {failed, Reason}
after 10000 ->
    timeout
end.
```
...
```erlang
{ok, Token} = bondy_connect_client:call_async(Conn, <<"com.example.long">>, []),
ok = bondy_connect_client:cancel(Conn, Token),            %% killnowait
%% or: bondy_connect_client:cancel(Conn, Token, skip)
receive {bondy_connect_client, Token, {error, _}} -> cancelled end.
```
...
{ok, RegId} = bondy_connect_client:register(Conn, <<"com.example.echo">>, Echo),

%% later
ok = bondy_connect_client:unregister(Conn, RegId).        %% by id …
%% ok = bondy_connect_client:unregister(Conn, <<"com.example.echo">>).  %% … or by URI
```
...
{ok, _} = bondy_connect_client:register(Conn, <<"com.example.divide">>, Divide).
```
...
```erlang
%% Fire-and-forget
ok = bondy_connect_client:publish(Conn, <<"com.example.events">>, [<<"tick">>]),

%% With keyword args
ok = bondy_connect_client:publish(Conn, <<"com.example.events">>, [<<"tick">>], #{seq => 1}),

%% Acknowledged
{ok, PubId} = bondy_connect_client:publish(
...
{ok, SubId} = bondy_connect_client:subscribe(Conn, <<"com.example.events">>, Handler),

receive
    {event, Args} -> handle(Args)
end,

ok = bondy_connect_client:unsubscribe(Conn, SubId).       %% by id, or by topic URI
```
...
One deliberate exception: a **transient** router `ABORT` retries even on the
first connect, regardless of `retry_initial_connect` — see
[Router refusals](#router-refusals-transient-vs-permanent-abort) below.

### Router refusals — transient vs permanent ABORT

A router may refuse a handshake with an `ABORT`. **Not every refusal is fatal**,
and treating them alike is the single most common way a client turns a moment of
router load into a self-inflicted outage.

Bondy tells you which kind you got. Every Bondy `ABORT` carries a `nature` key in
its details:

| `nature` | Meaning | What a client must do |
| --- | --- | --- |
| `transient` | Nothing is wrong with the request; retrying it **unchanged** could succeed. | Back off and retry. |
| `permanent` | The request itself is at fault and will fail identically forever. | Surface the error. Do **not** retry. |

The one you will meet at scale is the **HELLO load-admission gate**. When a
node's run queues are deep, an accepted session would spend seconds in
scheduling delay and probably time out on the client anyway — after holding a
socket, session state and auth work the whole time. So the router refuses at the
door instead, which costs a parse and one encoded frame:

```erlang
#abort{
    reason_uri = <<"wamp.error.unavailable">>,
    details    = #{<<"nature">>  => <<"transient">>,
                   <<"code">>    => <<"S003">>,
                   <<"message">> => <<"The router is overloaded and cannot "
                                      "accept new sessions at the moment. "
                                      "Please retry.">>}
}
```

This is shedding, not failure. The node is healthy and serving its admitted
sessions well — it is declining to make that worse. A client that gives up here
converts a recoverable condition into a permanent one, and a fleet that all
gives up at once turns a load spike into an outage.

**`bondy_connect_sdk` handles this for you.** A `transient` ABORT is routed into the
same bounded, backed-off reconnect loop as a dropped link — including on the
**first** connect, where `retry_initial_connect => false` would otherwise apply.
That default exists so a *misconfigured* `connect/1` (wrong URL, wrong realm, bad
credentials) fails fast instead of vanishing into a backoff loop; a transient
ABORT is the opposite of a misconfiguration, and fleet startup against a busy
cluster is overwhelmingly a first connect. A `permanent` ABORT still fails fast,
exactly as before.

Classification uses `nature` when present and falls back to an allow-list of
unambiguous availability URIs (`wamp.error.unavailable`,
`bondy.error.unavailable`) for a router too old to send it, or a non-Bondy WAMP
router. An ABORT that cannot be classified stays fatal — retrying a genuinely
permanent failure forever is worse than surfacing it.

Tune the aggressiveness through the same `reconnect` map. Spreading retries is
what stops a whole fleet re-arriving in lockstep and re-tripping the gate:
...
#{reconnect => #{
      ...
      backoff_min     => 1000,
      backoff_max     => 60000,
      deadline        => 0        %% 0 = keep trying; the gate WILL open
  }}
```

> **Writing a client in another language?** Implement the same rule: on `ABORT`,
> read `details.nature`. Retry `transient` with exponential backoff **and
> jitter**; surface `permanent`. If you cannot read `nature`, treat
> `wamp.error.unavailable` as retryable. Retrying without backoff is worse than
> not retrying at all — an un-backed-off retry storm is itself what keeps the
> run queues deep.
...
{ok, R} = bondy_connect_client:call(Conn, <<"bondy.session.self">>, []).
```
...
init([]) ->
    {ok, _} = application:ensure_all_started(bondy_connect_sdk),
    {ok, Conn} = bondy_connect_client:connect(price_conn, #{
        transport => tcp,
        endpoint  => {"127.0.0.1", 18082},
        realm     => <<"com.example.realm">>,
        auth      => #{method => <<"anonymous">>}
    ...
    {ok, _RegId} = bondy_connect_client:register(Conn, <<"com.example.quote">>, Quote),

    %% Subscribe to a topic; print each event.
    OnTick = fun(Args, _KWArgs, _Details) ->
        logger:info("tick: ~p", [Args])
    end,
    {ok, _SubId} = bondy_connect_client:subscribe(Conn, <<"com.example.ticks">>, OnTick),

    {ok, #{conn => Conn}}.

handle_call({latest, Symbol}, _From, #{conn := Conn} = State) ->
    Reply =
        case bondy_connect_client:call(Conn, <<"com.example.quote">>, [Symbol]) of
...
An `ABORT` ends a handshake. Whether that is fatal is decided by
`Details`' `nature` key, **not** by the URI:

| `nature` | Handled by `bondy_connect_sdk` as | Examples |
|---|---|---|
...
You do not need to implement this yourself when using `bondy_connect_sdk`. You **do**
need to implement it in a client written against the raw protocol — see
[Router refusals](#router-refusals-transient-vs-permanent-abort).
...
{ok, Conn} = bondy_connect_client:connect(Name, Spec).
Conn       = bondy_connect_client:named(Name).
Status     = bondy_connect_client:status(Conn).   %% connecting|establishing|established|reconnecting|down
ok         = bondy_connect_client:disconnect(Conn).

%% Caller
{ok, R} | {error, _} = bondy_connect_client:call(Conn, Uri[, Args[, KWArgs[, Opts]]]).
{ok, Token}          = bondy_connect_client:call_async(Conn, Uri, Args[, KWArgs[, Opts]]).
ok | {error, _}      = bondy_connect_client:cancel(Conn, Token[, Mode]).  %% skip|kill|killnowait

%% Callee
{ok, RegId} = bondy_connect_client:register(Conn, Uri, Handler[, Opts]).
ok          = bondy_connect_client:unregister(Conn, RegId | Uri).

%% Publisher
ok | {ok, PubId} = bondy_connect_client:publish(Conn, Topic, Args[, KWArgs[, Opts]]).

%% Subscriber
{ok, SubId} = bondy_connect_client:subscribe(Conn, Topic, Handler[, Opts]).
ok          = bondy_connect_client:unsubscribe(Conn, SubId | Topic).
```
...
arguments into a call. For per-function detail see the `m:bondy_connect_client` module
