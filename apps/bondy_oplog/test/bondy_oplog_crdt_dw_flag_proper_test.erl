...
%% PropEr properties for the Disable-Wins Flag (DWFlag), tier_2. A command
%% sequence over a few origins (`{enable,O}`, `{disable,O}`,
%% `{sync,From,To}`) is simulated with realistic causal delivery. Beyond
%% convergence, `prop_disable_wins_oracle` pins the semantics: the flag is
%% `true` iff some enable observed every disable (its context dominates the
%% disable frontier) — concurrent disable wins.

-module(bondy_oplog_crdt_dw_flag_proper_test).

-include_lib("proper/include/proper.hrl").
-include_lib("eunit/include/eunit.hrl").

-define(MOD, bondy_oplog_crdt_dw_flag).
...
-export([prop_disable_wins_oracle/0]).
...
%% Disable-wins semantics oracle: the flag is true iff some enable has a
%% context that dominates the disable frontier (it observed every disable).
prop_disable_wins_oracle() ->
    ?FORALL(Cmds, cmds_gen(), begin
        {_PerOrigin, Log} = simulate(Cmds),
        State = ?MOD:interpret_cog(Log, ?MOD:init()),
...
%% =============================================================================
%% Disable-wins oracle (independent of the CRDT implementation)
%% =============================================================================
...
oracle(Log) ->
    ...
    Frontier = lists:foldl(
        fun(Ev, Acc) ->
            ?CORE:vv_merge(Acc, [?CORE:dot_of(bondy_oplog_event:key(Ev))])
        end,
        [],
        [Ev || Ev <- Log, disable =:= bondy_oplog_crdt_commutative:op_of(Ev)]
    ),
    lists:any(fun(Ctx) -> ?RW:vv_dominates(Ctx, Frontier) end, Enables).
