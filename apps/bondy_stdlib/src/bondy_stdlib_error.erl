...
is_type(#{
    ...
    source := _,
    doc_uri := _
}) when is_atom(T), is_map(Details), is_list(Causes) ->
    true;
is_type(_) ->
    false.
...
normalize(List) when is_list(List) ->
    [normalize(X) || X <- List];
normalize({Msg, Opts}) when
    is_map(Opts), (is_list(Msg) orelse is_binary(Msg))
->
...
attrs(Error, UserMsg) when is_binary(UserMsg); is_list(UserMsg) ->
    ...
    Base#{
        message := <<
            (iolist_to_binary(Msg0))/binary,
            $\s,
            (iolist_to_binary(UserMsg))/binary
        >>
    }.
...
attrs(inconsistency_error) ->
    #{
        ...
        nature => permanent,
        message =>
            ~"The values provided for the a set of properties or keys are inconsistent."
    };
attrs(conflict) ->
    #{code => ~"C006", message => ~""};
%% Clustering
...
attrs(invalid_schema) ->
    #{
        code => ~"D055",
        nature => permanent,
        message => ~"Schema not found or invalid"
    };
