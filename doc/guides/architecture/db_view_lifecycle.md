...
| The liveness trade | Confirmations are recency-filtered: a peer silent past `db.aae.peer_timeout` stops holding the frontier back, so one dead node cannot freeze reclamation cluster-wide. The deliberate consequence: a long-silent node's needed history can be compacted away everywhere. |
