...
Bondy provides per-listener configuration for CORS (Cross-Origin Resource Sharing) and standard HTTP security response headers. Every key documented below exists for any listener whose `protocol` is `http`, under that listener's own name:

```
listeners.<name>.cors.*
listeners.<name>.security_headers.*
```

The examples use a listener named `public_http`. Substitute your own name; there is no fixed set of listeners and no global block to fall back to — see [Listeners](listeners.md).
...
> `Access-Control-Allow-Credentials` is set to `true`. The cookies also need to
> be scoped to the shared parent domain and sent on cross-site requests, which
> is `cookie_domain` and `cookie_same_site` in the OIDC provider config.
...
#### `listeners.$name.cors.allowed_methods`

The value for the `Access-Control-Allow-Methods` response header. A comma-separated list of HTTP methods.

```
listeners.public_http.cors.allowed_methods = GET,HEAD,OPTIONS,POST,PUT,PATCH,DELETE
```

Default: `GET,HEAD,OPTIONS,POST,PUT,PATCH,DELETE`

#### `listeners.$name.cors.allowed_headers`
...
```
listeners.public_http.cors.allowed_headers = origin,x-requested-with,content-type,accept,authorization,accept-language,x-csrf-token
```

Default: `origin,x-requested-with,content-type,accept,authorization,accept-language,x-csrf-token`

#### `listeners.$name.cors.max_age`
...
```
listeners.public_http.cors.max_age = 86400
```
...
listeners.public_http.cors.allowed_origins = https://app.example.com, https://admin.example.com
listeners.public_http.cors.allowed_methods = GET,HEAD,OPTIONS,POST,PUT,PATCH,DELETE
listeners.public_http.cors.allowed_headers = origin,x-requested-with,content-type,accept,authorization
listeners.public_http.cors.max_age = 86400
```
...
#### `listeners.$name.security_headers.enabled`
...
#### `listeners.$name.security_headers.hsts`
...
#### `listeners.$name.security_headers.frame_options`
...
#### `listeners.$name.security_headers.content_type_options`
...
```
listeners.public_http.security_headers.content_type_options = nosniff
```

Default: `nosniff`

#### `listeners.$name.security_headers.content_security_policy`
...
```
listeners.public_http.security_headers.content_security_policy = default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'
```
...
```
listeners.public_http.server_header = bondy
```
...
listeners.public_http.security_headers.enabled = on
listeners.public_http.security_headers.hsts = max-age=31536000; includeSubDomains
listeners.public_http.security_headers.frame_options = DENY
listeners.public_http.security_headers.content_type_options = nosniff
listeners.public_http.security_headers.content_security_policy = default-src 'self'; frame-ancestors 'none'

## Suppress server version
listeners.public_http.server_header =
```
